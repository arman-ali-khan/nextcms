...
const AuthContext = ({ children }) => {
    ...
    if (cookieEmail) {
      ...
      axios
        .get(
          ...
          }/api/siteuser?email=${cryptr?.decrypt(cookieEmail)}&siteurl=${siteId.siteurl}`
        )
        .then((res) => {
  ...
  useEffect(() => {
    if (getSiteParams) {
      ...
      axios
        .get(
          `${process.env.NEXT_PUBLIC_LOCAL}/api/siteurl?siteurl=${getSiteParams}`
        )
        .then((res) => {
          setSiteId(res.data);
          setSiteLoading(false);
  ...
  useEffect(() => {
    if (user?.email) {
      setMainUserLoading(false);
      axios
        .get(`${process.env.NEXT_PUBLIC_LOCAL}/api/site?uid=${user?.uid}`)
        .then((res) => {
          setUserId(res.data);
          setMainUserLoading(false);
        });
